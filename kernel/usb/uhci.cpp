...
static void ms_delay(uint32_t ms) {
    for (uint32_t i = 0; i < ms * 1000; i++) io_delay();
}
...
alignas(4096) static uint32_t s_frame_list_buf[FRAME_COUNT];
alignas(16)   static QH       s_qh_pool_buf[UHCI_QH_COUNT];
alignas(16)   static TD       s_td_pool_buf[UHCI_TD_COUNT];
alignas(64)   static uint8_t  s_buf_pool_buf[UHCI_TD_COUNT * 64];
static uint32_t* s_frame_list = s_frame_list_buf;
static QH*       s_int_qh     = &s_qh_pool_buf[0];
static QH*       s_ctrl_qh    = &s_qh_pool_buf[1];
static TD*       s_td_pool    = s_td_pool_buf;
static uint8_t*  s_buf_pool   = s_buf_pool_buf;
...
bool uhci_init() {
    ...
    pci_write16(pci.bus, pci.dev, pci.func, 0xC0, 0xAF00); 
    ...
    for (int port = 0; port < 8; port++) {
        uint16_t reg = port * 2 + UHCI_PORTSC0;
        uint16_t val = inw(s_iobase + reg);
        if (val == 0xFFFF) break; // End of ports
        port_reset(reg);
    }
...
void uhci_port_reset(uint8_t port) {
    port_reset(port * 2 + UHCI_PORTSC0);
}
uint8_t uhci_irq() { return s_irq; }
bool uhci_port_connected(uint8_t port) {
    uint16_t reg = port * 2 + UHCI_PORTSC0;
    uint16_t val = inw(s_iobase + reg);
    if (val == 0xFFFF) return false;
    return (val & PORT_CCS) != 0;
}
...
uint8_t uhci_register_interrupt(uint8_t addr, uint8_t endpoint,
                                bool dir_in, uint8_t max_packet,
                                uint8_t interval, UhciPollCb cb)
{
    if (s_pipe_count >= UHCI_TD_COUNT - 3) return 0;
    ...
    td->token    = (dir_in ? TD_TOKEN_PID_IN : TD_TOKEN_PID_OUT)
                 | TD_TOKEN_DEVADDR(addr)
                 | TD_TOKEN_ENDP(endpoint)
                 | TD_TOKEN_TOGGLE(0)
                 | TD_TOKEN_MAXLEN(max_packet - 1);
    ...
    if (sts & STS_USBINT) {
        for (int i = 0; i < s_pipe_count; i++) {
            ...
            if (!(td->ctrl_sts & (TD_CS_STALLED | TD_CS_DBUF_ERR | TD_CS_BABBLE))) {
                int actual = (td->ctrl_sts + 1) & TD_CS_ACTLEN_MASK;
                if (actual > 0 && p.cb)
                    p.cb(p.buf, (uint8_t)actual);
            }
            p.toggle ^= 1;
            td->token = (p.dir_in ? TD_TOKEN_PID_IN : TD_TOKEN_PID_OUT)
                      | TD_TOKEN_DEVADDR(p.dev_addr)
                      | TD_TOKEN_ENDP(p.endpoint)
                      | TD_TOKEN_TOGGLE(p.toggle)
                      | TD_TOKEN_MAXLEN(p.max_packet - 1);
            td->ctrl_sts = TD_CS_ACTIVE | TD_CS_IOC | TD_CS_SPD | (3u << TD_CS_ERR_SHIFT);
        }
    }
}
...
uint32_t ip_from_str(const char* s) {
    ...
    for (int i = 0; i < 4; i++) {
        uint32_t octet = 0;
        while (*s >= '0' && *s <= '9') octet = octet * 10 + (*s++ - '0');
        if (*s == '.') s++;
        r = (r << 8) | (octet & 0xFF);
    }
    return htonl(r);
...
void ip_to_str(uint32_t ip, char* out) {
    ...
    for (int i = 3; i >= 0; i--) {
        ...
        if (o >= 100) { out[pos++] = '0' + o/100; o %= 100; tmp[0]=1; }
        else tmp[0]=0;
        if (o >= 10 || tmp[0]) { out[pos++] = '0' + o/10; o %= 10; }
        out[pos++] = '0' + o;
        if (i > 0) out[pos++] = '.';
    }
    out[pos] = 0;
...
bool net_arp_lookup(uint32_t ip, uint8_t mac_out[6]) {
    for (int i = 0; i < ARP_CACHE_SIZE; i++) {
        if (s_arp_cache[i].valid && s_arp_cache[i].ip == ip) {
            kmemcpy(mac_out, s_arp_cache[i].mac, 6);
            return true;
        }
    }
    return false;
...
static void handle_icmp(const IpHeader* ip, const uint8_t* data, uint16_t len) {
    ...
    if (icmp->type == ICMP_ECHO_REQUEST) {
        ...
        EthHeader* eth = (EthHeader*)pkt;
        if (!net_arp_lookup(ip->src_ip, eth->dst)) {
            kmemset(eth->dst, 0xFF, 6);
        }
        kmemcpy(eth->src, s_my_mac, 6);
        ...
        rip->ver_ihl   = 0x45;
        rip->dscp_ecn  = 0;
        rip->total_len = htons(ip_total);
        rip->id        = htons(s_ip_id++);
        rip->flags_frag= 0;
        rip->ttl       = 64;
        ...
        uint16_t payload = (uint16_t)(len - sizeof(IcmpHeader));
        if (payload > 0)
            kmemcpy((uint8_t*)ricmp + sizeof(IcmpHeader),
                    data + sizeof(IcmpHeader), payload);
...
static void handle_udp(const IpHeader* ip, const uint8_t* data, uint16_t len) {
    ...
    uint16_t payload_len = (uint16_t)(ntohs(udp->length) - sizeof(UdpHeader));

    for (int i = 0; i < UDP_HANDLERS; i++) {
        if (s_udp_handlers[i].used && s_udp_handlers[i].port == dst_port) {
            s_udp_handlers[i].cb(ip->src_ip, src_port, dst_port,
                                  payload, payload_len);
            return;
        }
    }
...
static void handle_ip(const uint8_t* data, uint16_t len) {
    ...
    if (ip->dst_ip != s_my_ip && ip->dst_ip != bcast &&
        ip->dst_ip != 0xFFFFFFFF) return;
...
void net_receive(const uint8_t* data, uint16_t len) {
    ...
    uint16_t payload_len = (uint16_t)(len - sizeof(EthHeader));

    switch (type) {
        case ETH_TYPE_ARP: handle_arp(payload, payload_len); break;
        case ETH_TYPE_IP:  handle_ip(payload, payload_len);  break;
        default: break;
    }
}

...
static void memset32(uint32_t* ptr, uint32_t val, uint32_t count) {
    for (uint32_t i = 0; i < count; i++) ptr[i] = val;
}
...
void paging_map(uint32_t virt, uint32_t phys, uint32_t flags) {
    ...
    if (!(page_directory[pd_idx] & PAGE_PRESENT)) {
        ...
        page_directory[pd_idx] = (uint32_t)new_pt | PAGE_PRESENT | PAGE_WRITE;
    }
